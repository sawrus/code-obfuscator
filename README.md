...
- Глубокая SQL-обфускация идентификаторов таблиц и полей (включая квалифицированные ссылки: `r.user_id`).
...
## Примеры глубокой обфускации

- SQL: `FROM refill r` -> `FROM test666 r`, `user_id, amount, code` -> `a1, b1, c1` (при наличии правил в mapping).
- Python: `PG_MWL_PASSWORD` -> `PG_CAT_P`, `get_suspect_users_from_refill_actions` -> `get_a_b_c`.
- Python imports guard: `from apiutil.models import User` и `class Falcon8382(User):` сохраняют `User` без обфускации.
